...
/* 品牌名集中在这里 —— 默认值与「改名迁移」共用同一份，不然很容易
...
const state={
  ...
  book:{
    ...
    spine:BRAND.spine, cover:'#e8e0d2', paper:'#fffdf8', ink:'#2b2620',
...
magazine:{tone:'photo',name:'杂志',hint:'封面标题 · 期号 · 条码',
  draw(c){
    ...
    ctx.fillText('VOLUME ONE · KADA PRESS',W*.07,H*.115);
    ctx.globalAlpha=1;
...
kraft:{tone:'kraft',name:'牛皮纸',hint:'胶带 · 手写日期 · 手账感',
  draw(c){
    ...
    ctx.fillText('KADA STUDIO',W*.79,H*.906);
...
function renderCover(W,H){
  ...
  x.globalAlpha=1;
  const ts=fitFont(x,bk.title||BRAND.name,cw,FONTS.serif,H*.100,H*.030,'700');
  /* 题字工艺跟着材料走：布面 / 纸面 → 压凹（字陷进材料里）；
     丝绸 / 皮革 / 硬壳 → 烫金。这是封面"质感"最集中的一处 ——
     有工艺的字和直接印上去的字，是两本书。 */
  pressTreat(x,bk.title||BRAND.name,cx,H*.188,ts,ts*.10,FONTS.serif,null,ink);
  x.save();
...
function renderBack(W,H){
  ...
  const ts=fitFont(x,bk.title||BRAND.name,W*.72,FONTS.serif,H*.055,H*.026,'600');
  x.fillStyle=bk.ink; x.globalAlpha=.92;
  x.font='600 '+R(ts)+'px '+FONTS.serif;
  tracked(x,bk.title||BRAND.name,W/2,H*.400,ts*.09,'center');
  x.globalAlpha=.44;
  const es=R(W*.0165);
  x.font=R(es)+'px '+FONTS.sans;
  ...
  x.fillText('KADA STUDIO · 2026',W/2,y+bh+W*.036);
...
function shareSheet(){
  ...
  const ts=fitFont(x,state.book.title||BRAND.name,W*.6,FONTS.serif,W*.082,W*.030,'700');
  x.fillStyle=soft(1);
  x.font='700 '+R(ts)+'px '+FONTS.serif;
  tracked(x,state.book.title||BRAND.name,W/2,head*.58,ts*.10,'center');
...
function exportSheet(){
  ...
  const name='咔哒书-分享长图-'+s.CW+'x'+s.CH+'.png';
...
async function exportCurrent(){
  ...
  const fmt=state.spec.format==='png'?'png':(state.spec.format==='webp'?'webp':'jpeg');
  const blob=await canvasBlob(g.canvas,fmt,state.spec.quality);
  if(blob) downloadBlob(blob,'咔哒书-'+state.tpl+'-'+g.canvas.width+'x'+g.canvas.height+'.'+(fmt==='jpeg'?'jpg':fmt));
  toast('当前成片已下载');
}